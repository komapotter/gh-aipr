...
func resolveProvider(getenv EnvLookup, file FileConfig) (string, string, error) {
	if v, ok := getenv(EnvProvider); ok && strings.TrimSpace(v) != "" {
...
// `gh aipr auth register` while retaining the underlying error.
...
// Status is a non-secret snapshot for `gh aipr auth status`.
type Status struct {
