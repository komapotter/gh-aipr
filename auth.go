...
Run "gh aipr auth <command> -h" for command help.
...
func (c *authCLI) register(args []string) error {
	fs := c.newFlagSet("gh aipr auth register", func(w io.Writer) {
		fmt.Fprint(w, `Usage: gh aipr auth register [options] [provider]
...
`)
	...
	fmt.Fprintf(c.stdout, "Active provider: %s. Use gh aipr auth switch to change.\n", fileCfg.ModelProvider)
	return nil
}

func (c *authCLI) remove(args []string) error {
	fs := c.newFlagSet("gh aipr auth remove", func(w io.Writer) {
		fmt.Fprint(w, `Usage: gh aipr auth remove [options] [provider]
...
func (c *authCLI) status(args []string) error {
	fs := c.newFlagSet("gh aipr auth status", func(w io.Writer) {
		fmt.Fprint(w, `Usage: gh aipr auth status
...
func (c *authCLI) switchProvider(args []string) error {
	fs := c.newFlagSet("gh aipr auth switch", func(w io.Writer) {
		fmt.Fprint(w, `Usage: gh aipr auth switch [options] [provider]

Set the active provider to openai or anthropic. This updates local
config (~/.config/gh-aipr/config.yml) and does not change API keys.
...
`)
	...
	fmt.Fprintf(c.stdout, "Note: AI_PROVIDER in the environment overrides this setting.\n")
	return nil
}
